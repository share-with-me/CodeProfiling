...
const struct xlat sigchld_codes[] = {
 { (unsigned)(CLD_EXITED), "CLD_EXITED" },
 { (unsigned)(CLD_KILLED), "CLD_KILLED" },
 { (unsigned)(CLD_DUMPED), "CLD_DUMPED" },
 { (unsigned)(CLD_TRAPPED), "CLD_TRAPPED" },
 { (unsigned)(CLD_STOPPED), "CLD_STOPPED" },
 { (unsigned)(CLD_CONTINUED), "CLD_CONTINUED" },
 { 0, 0 }
};
static
...
const struct xlat sigpoll_codes[] = {
 { (unsigned)(POLL_IN), "POLL_IN" },
 { (unsigned)(POLL_OUT), "POLL_OUT" },
 { (unsigned)(POLL_MSG), "POLL_MSG" },
 { (unsigned)(POLL_ERR), "POLL_ERR" },
 { (unsigned)(POLL_PRI), "POLL_PRI" },
 { (unsigned)(POLL_HUP), "POLL_HUP" },
 { 0, 0 }
};
static
...
printsigsource(const siginfo_t *sip)
{
 tprintf(", si_pid=%u, si_uid=%u",
  (unsigned int) sip->_sifields._kill.si_pid,
  (unsigned int) sip->_sifields._kill.si_uid);
}
static void
printsigval(const siginfo_t *sip)
{
 tprintf(", si_value={int=%d, ptr=", sip->_sifields._rt.si_sigval.sival_int);
 printaddr((unsigned long) sip->_sifields._rt.si_sigval.sival_ptr);
 tprints("}");
}
static void
 ...
 if (((sip)->si_code <= 0)) {
  ...
  case SI_TIMER:
   tprintf(", si_timerid=%#x, si_overrun=%d",
    sip->_sifields._timer.si_tid, sip->_sifields._timer.si_overrun);
   printsigval(sip);
   break;
  default:
   printsigsource(sip);
  ...
  case 17:
   ...
   if (sip->si_code == CLD_EXITED)
    tprintf("%d", sip->_sifields._sigchld.si_status);
   else
    printsignal(sip->_sifields._sigchld.si_status);
  ...
  case 11: case 7:
   tprints(", si_addr=");
   printaddr((unsigned long) sip->_sifields._sigfault.si_addr);
   break;
  case 29:
   switch (sip->si_code) {
   case POLL_IN: case POLL_OUT: case POLL_MSG:
    tprintf(", si_band=%ld",
     (long) sip->_sifields._sigpoll.si_band);
    break;
   }
   break;
  case 31:
   tprints(", si_call_addr=");
   printaddr((unsigned long) sip->_sifields._sigsys._call_addr);
   tprintf(", si_syscall=__NR_%s, si_arch=",
    syscall_name((unsigned) sip->_sifields._sigsys._syscall));
   printxval(audit_arch, sip->_sifields._sigsys._arch, "AUDIT_ARCH_???");
   break;
  default:
