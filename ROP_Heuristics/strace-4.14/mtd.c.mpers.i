...
const struct xlat mtd_mode_options[] = {
 { (unsigned)(MTD_OPS_PLACE_OOB), "MTD_OPS_PLACE_OOB" },
 { (unsigned)(MTD_OPS_AUTO_OOB), "MTD_OPS_AUTO_OOB" },
 { (unsigned)(MTD_OPS_RAW), "MTD_OPS_RAW" },
 { 0, 0 }
};
static
const struct xlat mtd_file_mode_options[] = {
 { (unsigned)(MTD_FILE_MODE_NORMAL), "MTD_FILE_MODE_NORMAL" },
 { (unsigned)(MTD_FILE_MODE_OTP_FACTORY), "MTD_FILE_MODE_OTP_FACTORY" },
 { (unsigned)(MTD_FILE_MODE_OTP_USER), "MTD_FILE_MODE_OTP_USER" },
 { (unsigned)(MTD_FILE_MODE_RAW), "MTD_FILE_MODE_RAW" },
 { 0, 0 }
};
static
const struct xlat mtd_type_options[] = {
  { (unsigned)(0), "MTD_ABSENT" },
  { (unsigned)(1), "MTD_RAM" },
  { (unsigned)(2), "MTD_ROM" },
  { (unsigned)(3), "MTD_NORFLASH" },
  { (unsigned)(4), "MTD_NANDFLASH" },
  { (unsigned)(6), "MTD_DATAFLASH" },
  { (unsigned)(7), "MTD_UBIVOLUME" },
  { (unsigned)(8), "MTD_MLCNANDFLASH" },
 { 0, 0 }
};
static
...
const struct xlat mtd_otp_options[] = {
  { (unsigned)(0), "MTD_OTP_OFF" },
  { (unsigned)(1), "MTD_OTP_FACTORY" },
  { (unsigned)(2), "MTD_OTP_USER" },
 { 0, 0 }
};
static
const struct xlat mtd_nandecc_options[] = {
  { (unsigned)(0), "MTD_NANDECC_OFF" },
  { (unsigned)(1), "MTD_NANDECC_PLACE" },
  { (unsigned)(2), "MTD_NANDECC_AUTOPLACE" },
  { (unsigned)(3), "MTD_NANDECC_PLACEONLY" },
  { (unsigned)(4), "MTD_NANDECC_AUTOPL_USR" },
 { 0, 0 }
};
static void
...
decode_mtd_oob_buf64(struct tcb *tcp, const long addr)
{
 ...
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&mbuf64)), (void *) (&mbuf64)))
  return;
 tprintf("{start=%#" "l" "x" ", length=%#x, usr_ptr=%#" "l" "x" "}",
...
decode_mtd_write_req(struct tcb *tcp, const long addr)
{
 ...
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&mreq)), (void *) (&mreq)))
  return;
 tprintf("{start=%#" "l" "x" ", len=%#" "l" "x"
  ", ooblen=%#" "l" "x" ", usr_data=%#" "l" "x"
  ", usr_oob=%#" "l" "x" ", mode=",
...
decode_nand_oobinfo(struct tcb *tcp, const long addr)
{
 ...
 tprints(", oobfree={");
 for (i = 0; i < (sizeof(ninfo.oobfree) / sizeof((ninfo.oobfree)[0])); ++i) {
  if (i)
   tprints("}, ");
  tprints("{");
  for (j = 0; j < (sizeof(ninfo.oobfree[0]) / sizeof((ninfo.oobfree[0])[0])); ++j) {
   if (j)
    tprints(", ");
 ...
 tprints("}}, eccpos={");
 for (i = 0; i < (sizeof(ninfo.eccpos) / sizeof((ninfo.eccpos)[0])); ++i) {
  if (i)
   tprints(", ");
...
decode_nand_ecclayout_user(struct tcb *tcp, const long addr)
{
 ...
 for (i = 0; i < (sizeof(nlay.eccpos) / sizeof((nlay.eccpos)[0])); ++i) {
  if (i)
   tprints(", ");
  tprintf("%#x", nlay.eccpos[i]);
 }
 tprintf("}, oobavail=%#x, oobfree={", nlay.oobavail);
 for (i = 0; i < (sizeof(nlay.oobfree) / sizeof((nlay.oobfree)[0])); ++i) {
  if (i)
   tprints(", ");
...
MPERS_PRINTER_DECL(int, mtd_ioctl, struct tcb *tcp, const unsigned int code, const long arg)
{
 switch (code) {
