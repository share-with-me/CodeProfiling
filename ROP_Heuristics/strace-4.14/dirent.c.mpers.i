...
{
 kernel_dirent d;
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&d)), (void *) (&d)))
  return;
 tprintf("{d_ino=%llu, d_off=%llu, d_reclen=%u, d_name=",
  (sizeof(d.d_ino) == sizeof(char) ? (unsigned long long) (unsigned char) (d.d_ino) : sizeof(d.d_ino) == sizeof(short) ? (unsigned long long) (unsigned short) (d.d_ino) : sizeof(d.d_ino) == sizeof(int) ? (unsigned long long) (unsigned int) (d.d_ino) : sizeof(d.d_ino) == sizeof(long) ? (unsigned long long) (unsigned long) (d.d_ino) : (unsigned long long) (d.d_ino)),
  (sizeof(d.d_off) == sizeof(char) ? (unsigned long long) (unsigned char) (d.d_off) : sizeof(d.d_off) == sizeof(short) ? (unsigned long long) (unsigned short) (d.d_off) : sizeof(d.d_off) == sizeof(int) ? (unsigned long long) (unsigned int) (d.d_off) : sizeof(d.d_off) == sizeof(long) ? (unsigned long long) (unsigned long) (d.d_off) : (unsigned long long) (d.d_off)), d.d_reclen);
 if (d.d_reclen > 256)
  d.d_reclen = 256;
 printpathn(tcp, addr + __builtin_offsetof (kernel_dirent, d_name), d.d_reclen);
 tprints("}");
}
...
int sys_getdents(struct tcb *tcp)
{
 ...
 if (((tcp)->u_error != 0) || !((tcp)->qual_flg & 0x004)) {
  printaddr(tcp->u_arg[1]);
  tprintf(", %u", count);
  return 0;
 }
 ...
 for (i = 0; len && i <= len - sizeof(kernel_dirent); ) {
  ...
  if (!((tcp)->qual_flg & 0x002)) {
   ...
   d_name_len -= __builtin_offsetof (kernel_dirent, d_name) + 1;
   if (d_name_len > 256)
    d_name_len = 256;
   tprintf("%s{d_ino=%llu, d_off=%llu, d_reclen=%u"
    ", d_name=", i ? ", " : "",
    (sizeof(d->d_ino) == sizeof(char) ? (unsigned long long) (unsigned char) (d->d_ino) : sizeof(d->d_ino) == sizeof(short) ? (unsigned long long) (unsigned short) (d->d_ino) : sizeof(d->d_ino) == sizeof(int) ? (unsigned long long) (unsigned int) (d->d_ino) : sizeof(d->d_ino) == sizeof(long) ? (unsigned long long) (unsigned long) (d->d_ino) : (unsigned long long) (d->d_ino)),
    (sizeof(d->d_off) == sizeof(char) ? (unsigned long long) (unsigned char) (d->d_off) : sizeof(d->d_off) == sizeof(short) ? (unsigned long long) (unsigned short) (d->d_off) : sizeof(d->d_off) == sizeof(int) ? (unsigned long long) (unsigned int) (d->d_off) : sizeof(d->d_off) == sizeof(long) ? (unsigned long long) (unsigned long) (d->d_off) : (unsigned long long) (d->d_off)),
    d->d_reclen);
